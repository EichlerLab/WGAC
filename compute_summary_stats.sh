...
cat temp1000_90percent.tab | awk '{print $1"\t"$2"\t"$3}' | grep -v "ML" | grep -v "QNV" | sort -k1,1V -k2,2n | bedtools merge -i stdin | wc -l
...
echo "number of pairsise alignments on same chromosome but at least 1 MB apart:"
